...
switch($mode)
{
	case 'LIST':
		...
		// build query
		$query = "SELECT id, responsetext, responsevalue FROM responselist";
		...
		if (!$results) {
			...
			if (mysql_num_rows($results) < 1) {
				...
				while($row = mysql_fetch_array($results)) {
					...
					echo '<td>' . $row['id'] . '</td>' . 
						 '<td>' . $row['responsetext'] . '</td>' . 
						 '<td>' . $row['responsevalue'] . '</td>' . 
						 '<td>' .
	...
	case 'DELETE':
		if (!isset($id)) {
			...
			$query = "DELETE FROM responselist WHERE id = '$id'";
	...
	case 'EDIT':
		...
		$query = "SELECT responsetext, responsevalue FROM responselist WHERE id='$id' LIMIT 1";
...
?>
		<form action="<?php echo $_SERVER['PHP_SELF'] . $querystring; ?>" method="post">
			<div>
				<label for="responsetext">Label Text:</label>
				<input type="text" name="responsetext" value="<?php echo $row['responsetext']; ?>" size="50" maxlength="60" />
			</div>
			<div>
				<label for="responsevalue">Value:</label>
				<input type="text" name="responsevalue" value="<?php echo $row['responsevalue']; ?>" size="50" maxlength="60" />
			</div>
