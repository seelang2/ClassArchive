...
if (isset($_POST['fromlogin']) && $_POST['fromlogin'] == 1) {
	...
	if (@mysql_num_rows($result) == 1) {
		...
		$_SESSION['statusmsg'] = "Error: Username and/or password is incorrect. $query";
		$_SESSION['redir'] = $me;
	...
	if (isset($_SESSION['UID']) && $_SESSION['UID'] != '' && isset($_SESSION['HASH']) && $_SESSION['HASH'] != '') {
		...
		$query = "SELECT id, name, password, permissions FROM user WHERE id = '$id'";
		...
		if (@mysql_num_rows($result) == 1) {
			...
			if ($hash == md5($row['id'] . $row['password'])) {
				if ($row['permissions'] >= $page_permission) {
					...
					$_SESSION['statusmsg'] = "Error: You have insufficient privileges to view that page.";
					$_SESSION['redir'] = $me;
					redirect(SITE_URL . DEFAULT_PAGE);
				}
				
			} else {
				$_SESSION['statusmsg'] = "Error: Invalid username and/or password.";
				$_SESSION['redir'] = $me;
				redirect(SITE_URL . LOGIN_PAGE);
			}
