...
function showDetails(id) {
	...
	*/
	...
	resultDiv.innerHTML = '<h2>Contact Details:</h2><p>' +
						  contacts[index].firstname + '<br />' +
						  contacts[index].lastname + '<br />' +
						  contacts[index].phone + '<br />' +
						  contacts[index].email + '</p>';
	
	// clearing a div using innerHTML
	// resultDiv.innerHTML = '';
	
	/*
	// clearing a div using dom methods
	while (resultDiv.hasChildNodes()) {
		resultDiv.removeChild(resultDiv.firstChild);
	}
	
	var tempElem = document.createElement("H2");
	resultDiv.appendChild(tempElem);
	tempElem.appendChild(document.createTextNode('Contact Details:'));
	
	var pElem = document.createElement("P");
	resultDiv.appendChild(pElem);
	
	pElem.appendChild(document.createTextNode(contacts[index].firstname));
	pElem.appendChild(document.createElement("BR"));
	
	pElem.appendChild(document.createTextNode(contacts[index].lastname));
	pElem.appendChild(document.createElement("BR"));
	
	pElem.appendChild(document.createTextNode(contacts[index].phone));
	pElem.appendChild(document.createElement("BR"));
	
	pElem.appendChild(document.createTextNode(contacts[index].email));
	*/
