...
/*
Latest updates
Changed XHR call flow to avoid race condition created by simultaneous requests as follows:
Every time 
...
function getResults(mode, callback, catid, prodid, page, respType) {
	...
	ajaxSendReq("GET", "/backend6a.php?mode=" + mode + "&catid=" + catid  + "&prodid=" + prodid + "&page=" + page + "&x=" + x, callback, null, respType, false);
...
function displayCats(results) {
	...
	selectCat.onchange = function() {
		...
		// Set current category to selected option
...
//		getResults('getresults', displayResults, selectCat.options[selectCat.selectedIndex].value, '', pageData.currentPage, 'XML');
		...
		getResults('count', setResultCount, pageData.currentCat, '', pageData.currentPage, 'TEXT');
