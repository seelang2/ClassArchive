 ...
 * POST		- Inserts a new resource into a collection.
...
@define('API_BASE_PATH', '/tc8516/backend/');

// A factor used for latency simulation (use 0 for no latency)
@define('LATENCY_FACTOR', 5);
...
/*
  ...
  
  {
	"tableName": {
		...
		"PK":{"field1":"value1","field2":"value2","field3":"value3"...},
		...
	},
	...
  }
  ...
  NOTE: Do NOT include an 'id' field as the PK is the array index and may be
  included automatically as 'id' in the return values.
  
  To relate tables together in a BelongsTo relationship, a FK field should 
  be added with the related table name. For example:

  {
	...
	"tableName2": {
		"0":["field1", "field2", "field3"...],
		"PK2":{
			"tableName1":"PK1",	<-- tableName2 belongs to tableName1
			"field1":"value1",
			"field2":"value2",
			"field3":"value3"
		}
  ...
  To relate tables together in a HABTM relationship, a separate array is used:
  ...
  NOTE: HABTM is not yet supported
...
switch($_SERVER['REQUEST_METHOD']) {
		...
		switch($pattern) {
			...
			case 'C':
				...
				if (!array_key_exists($ps[0], $data)) {
					...
					output(array('message'=>'Invalid collection'), 404);
				}
				
				$tmpData = $data[$ps[0]];
				array_shift($tmpData); // first record is fieldlist
				$result = array($ps[0] => array());
			...
			case 'CI':
				...
				if ( (!array_key_exists($ps[0], $data)) || 
					...
					output(array('message'=>'Resource not found'), 404);
				}
				
				$result = $data[$ps[0]][$ps[1]];
				$result['id'] = $ps[1];
				$result= array($ps[0] => $result);
			break;
			case 'CIC':
				$result = $data[$ps[0]][$ps[1]];
				$result['id'] = $ps[1];
			...
			$result = array($ps[0] => array());
				...
				$c2 = $data[$ps[1]];
				array_shift($c2);
				foreach($c1 as $id1 => $row1) {
					$row1['id'] = $id1;
					$row1[$ps[1]] = array();
					foreach($c2 as $id2 => $row2) {
						$row2['id'] = $id2;
						if ($row2[$ps[0]] == $id1) {
							$row1[$ps[1]][] = $row2;
						}
	...
	case 'PUT':
		...
		switch($pattern) {
			...
			break;
			case 'CI':
				// loop through 'table' fields and map request data
				foreach($data[$ps[0]][0] as $fieldName) {
